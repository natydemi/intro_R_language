...
# Este script traz uma versão simplificada de: Tipos e Estruturas de Dados no R -----
  ...
  #Os demais tipos de dados (complex e raw), assim como as demais estruturas 
  # (matriz, array, e listas), podem ser consultados no script "versao full"  


# Tipos de Dados no R  -----
    # No R existem alguns tipos de dados, que basicamente indicam como os objetos são armazenados:
    ...
    #Podemos também utilizar as funções `is.` e `as.` para testar e converter objetos, respectivamente:
        ...
        is.integer(x)
        #o objeto é um double (número contínuo)?
        is.double(x)
        
        #quero transformar em inteiro:
        x <- as.integer(x)
        is.integer(x)
